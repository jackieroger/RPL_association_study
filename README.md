...
Demo instructions: We have created a small synthetic EHR dataset (see folder synthetic_data/) to illustrate the expected input dataset format and how to analyze it using our code. To run the code on this data, open the Rmds in RStudio and run each one sequentially (starting with Rmd 01). Some Rmds run quite quickly (a couple minutes) and others can take longer (up to several hours) to complete. You can create the same file and folder naming structure that we did (with separate folders for data, figures, etc) or modify the code to impose a different structure. In general, the expected output of each Rmd is summarized by the Rmd's title. For details about all the expected outputs in each Rmd, review the comments throughout the Rmds.
